...
# List configurable categories
python anonimizar.py --lista-categorias

# Keep the normal coverage except selected categories
python anonimizar.py informe.docx --ley rgpd --excluir ubicacion dni-es

# Anonymize only selected categories
python anonimizar.py informe.docx --ley rgpd --incluir persona email telefono
```

`--incluir` and `--excluir` are mutually exclusive and apply only to
anonymization. The selected categories are recorded in the `.key.json` map.
Without either option, behavior is unchanged.
